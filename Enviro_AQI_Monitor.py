#!/usr/bin/env python3
# Enviro+ Environment Monitor with Sensor.community, Adafruit IO update
# and fetching ofr data, and MQTT lcd control
# Custom paths indicated by ## <>
# MQTT integration instructions: (e.g. Homeassistant) at end of code
...
logging.basicConfig(
    ...
    level=logging.INFO,
    datefmt='%Y-%m-%d %H:%M:%S')
logging.info("""Enviro_AQI_Monitor.py - Pimoroni Enviro+ with noise measurement (and optional SGP30) sensor capture and display.
 Supports external sensor capture and Sensor.community, (formerly Luftdaten, referenced in the code still as Luftdaten mqtt) 
 and Adafruit IO Updates
...
#https://devices.sensor.community/register and enter your Raspberry Pi
...
def retrieve_config():
    try:
        with open('/home/enviropi/enviro-monitor/Config/config.json', 'r') as f:  ##<> Enter location of your config file
    ...
    if 'outdoor_source_id' in parsed_config_parameters:
        outdoor_source_id = parsed_config_parameters['outdoor_source_id']  # Sets Sensor.community ("Luftdaten") or Adafruit IO Sensor IDs
        # if using those sensors for outdoor readings, with the format: {"Climate": id, "PM": id} for Sensor.community ("Luftdaten") or
    ...
    if 'enable_noise' in parsed_config_parameters: # Enables Noise level sensing
        ...
        enable_noise = False
    if 'enable_luftdaten_noise' in parsed_config_parameters: # Enables Noise level uploads to Sensor.community ("Luftdaten"). enable_noise must also be set to true for this to work
        enable_luftdaten_noise = parsed_config_parameters['enable_luftdaten_noise']
    else:
        enable_luftdaten_noise = False
    if 'disable_luftdaten_sensor_upload' in parsed_config_parameters: # Sensor.community ("Luftdaten") currently only supports two sensors per node
    ...
    if not enable_noise:
        ...
        if enable_luftdaten_noise:
            print('Noise sensor must be enabled in order to enable Sensor.community ("Luftdaten") Noise. Disabling SSensor.community ("Luftdaten") Noise')
            enable_luftdaten_noise = False
    else:
        if enable_luftdaten_noise and disable_luftdaten_sensor_upload == 'None':
            # Comment out next two lines once Sensor.community (Luftdaten) supports three sensors per node
            print('Sensor.community (Luftdaten) currently only supports two sensors and three have been enabled. Disabling Sensor.community ("Luftdaten") Climate uploads')
...
def read_climate_gas_values(luft_values, mqtt_values, own_data, maxi_temp, mini_temp, own_disp_values, gas_sensors_warm,
    ...
    if use_external_barometer == False:
        ...
        luft_values["pressure"] = "{:.2f}".format(raw_barometer * 100) # Send raw air pressure to Sensor.community ("Luftdaten"),
        ...
        own_disp_values["Bar"] = own_disp_values["Bar"][1:] + [[own_data["Bar"][1], 1]]
        mqtt_values["Bar"][0] = own_data["Bar"][1]
        # Remove altitude compensation from external barometer because Sensor.community ("Luftdaten") does its own altitude air pressure
        # compensation
...
def log_climate_and_gas(run_time, own_data, raw_red_rs, raw_oxi_rs, raw_nh3_rs, raw_temp, comp_temp, comp_hum,
    ...
    with open('/home/enviropi/enviro-monitor/envirolog', 'a') as f: ##<> Enter location of your log file
...
def send_to_luftdaten(luft_values, id, enable_particle_sensor, enable_noise, luft_noise_values, disable_luftdaten_sensor_upload):
    print("Sending Data to Sensor.community (Luftdaten)")
    ...
    if enable_particle_sensor and disable_luftdaten_sensor_upload != 'PM':
        ...
        try:
            resp_1 = requests.post("https://api.sensor.community/v1/push-sensor-data/",
                     json={
                         "software_version": "enviro_monitor " + monitor_version,
            ...
            print('Sensor.community PM Connection Error', e)
        except requests.exceptions.Timeout as e:
            resp_1_exception = True
            print('Sensor.community PM Timeout Error', e)
        except requests.exceptions.RequestException as e:
            resp_1_exception = True
            print('Sensor.community PM Request Error', e)
    ...
    if disable_luftdaten_sensor_upload != 'Climate':
        ...
        try:
            resp_2 = requests.post("https://api.sensor.community/v1/push-sensor-data/",
                     json={
                         "software_version": "enviro_monitor " + monitor_version,
                         "sensordatavalues": [{"value_type": key, "value": val} for
            ...
            print('Sensor.community Climate Connection Error', e)
        except requests.exceptions.Timeout as e:
            resp_2_exception = True
            print('Sensor.community Climate Timeout Error', e)
        except requests.exceptions.RequestException as e:
            resp_2_exception = True
            print('Sensor.community Climate Request Error', e)
    ...
    if enable_noise and enable_luftdaten_noise and luft_noise_values != []:
        ...
        print("Sending Sensor.community Noise Data", noise_values)
        noise_send_attempt = True
        try:
            resp_3 = requests.post("https://api.sensor.community/v1/push-sensor-data/",
                     json={
                         "software_version": "enviro_monitor " + monitor_version,
            ...
            print('Sensor.community Noise Connection Error', e)
        except requests.exceptions.Timeout as e:
            resp_3_exception = True
            print('Sensor.community Noise Timeout Error', e)
        except requests.exceptions.RequestException as e:
            resp_3_exception = True
            print('Sensor.community Noise Request Error', e)
        #print(resp_3)
...
def on_connect(client, userdata, flags, rc):
    es.print_update('Enviro+ Monitor Connected with result code ' + str(rc))
    client.subscribe('enviro-in/cmd/lcd') # Adding LCD MQTT control
    client.publish("enviro-in/MQTTclient/state", "ON") #MQTT LCD Control
    if enable_receive_data_from_homemanager:
...
def on_message(client, userdata, msg):
    if msg.topic == 'enviro-in/cmd/lcd': #MQTT LCD Control
        if str(msg.payload.decode("utf-8")) == 'OFF': #MQTT LCD Control
            disp.set_backlight(0) #MQTT LCD Control
            backlight_on = False #MQTT LCD Control
            client.publish("enviro-in/state/lcd", "OFF") #MQTT LCD Control
        if str(msg.payload.decode("utf-8")) == 'ON': #MQTT LCD Control
            disp.set_backlight(255) #MQTT LCD Control
            backlight_on = True #MQTT LCD Control
            client.publish("enviro-in/state/lcd", "ON")
        backlight_on = False #MQTT LCD Control
...
def display_all_aq(location, data, data_in_display_all_aq, enable_eco2_tvoc):
    ...
    for i in data_in_display_all_aq:
        ...
        if (i == "CO2" or i == "VOC") and location == "OUT" or data_value == None:
            ...
            # and no gas or P1 readings when Sensor.community ("Luftdaten") is the source of outdoor readings
    ...
    if enable_display:
        ...
        if selected_display_mode in own_data:
            if current_display_is_own and indoor_outdoor_function == 'Indoor' or selected_display_mode == "Bar":
                ...
                # No outdoor gas, P1, Lux graphs when outdoor source is Sensor.community ("Luftdaten").
...
def capture_external_outdoor_data(outdoor_source_type, outdoor_source_id, outdoor_aio_readings):
    ...
    if outdoor_source_type == "Adafruit IO":
        ...
        urls = {"Climate": 'https://data.sensor.community/airrohr/v1/sensor/{}/'.format(outdoor_source_id["Climate"]),
                "PM": 'https://data.sensor.community/airrohr/v1/sensor/{}/'.format(outdoor_source_id["PM"])}
        for url in urls:
            luft_error = False
            try:
                outdoor_luft_data = external_outdoor_data_session.get(urls[url], timeout=5).json()
                if not outdoor_luft_data:
                    raise ValueError("Empty response from Sensor.community API")
            except requests.exceptions.RequestException as outdoor_comms_error:
                luft_error = True
                print(f'Outdoor Sensor.community Error: {outdoor_comms_error}')
...
luft_values = {} # To be sent to Sensor.community ("Luftdaten")
...
# Sensor.commmunity ("Luftdaten") or Adafruit IO
maxi_temp = None
mini_temp = None
# When using an external outdoor sensor via Sensor.community ("Luftdaten") or Adafruit IO
outdoor_maxi_temp = None
outdoor_mini_temp = None

# Raspberry Pi ID to send to Sensor.commmunity ("Luftdaten")
...
luft_resp = True # Set to False when there is a Sensor.commmunity ("Luftdaten") comms error
...
# is triggered
comms_failure = False # Set to True when there has been a comms failure on either Sensor.commmunity ("Luftdaten") and/or Adafruit IO,
...
short_update_time = 0 # Set the short update time baseline (for watchdog alive file and Sensor.commmunity ("Luftdaten") updates)
short_update_delay = 5 # Time between short updates
...
long_update_delay = 5 # Time between long updates
long_update_toggle = False # Allows external outdoor Sensor.commmunity ("Luftdaten") or Adafruit updates to be undertaken every second long-update cycle
startup_stabilisation_time = 5 # Time to allow sensor stabilisation before sending external updates
...
try:
    with open('<Your Mender Software Version File Location Here>', 'r') as f: ##<> Enter location of your Mender file
...
try:
    with open('<Your Mender Config Version File Location Here>', 'r') as f: ##<> Enter location of your Mender file
...
persistent_data_log = {}
try:
    with open('/home/enviropi/enviro-monitor/log-persistent', 'r') as f: ##<> Enter location of your persistent log file
...
try:
    with noise_stream:
        while True:
                ...
                if noise_sample_counter != noise_previous_sample_count: # Only process new sample
                    ...
                    if noise_sample_counter > 10: # Wait for microphone stability
                        ...
                        if own_noise_ratio > 0:
                            ...
                            # Capture Max, Sensor.commmunity ("Luftdaten") and Adafruit IO sound levels and once display has been changed for > 2 seconds
                            if (time.time() - last_page) > 2:
                                if enable_luftdaten and enable_luftdaten_noise:
                                    luft_noise_values.append(round(noise_level, 2)) # Capture Sensor.commmunity ("Luftdaten") Noise Level
            ...
            # Read climate values, update Sensor.commmunity ("Luftdaten") and write to watchdog file every 2.5 minutes
            ...
            if time_since_short_update >= short_update_delay:
                ...
                print('Sensor.commmunity Values', luft_values)
                print('mqtt Values', mqtt_values)
                # Write to the watchdog file unless there is a comms failure for >= comms_failure_tolerance
                # when both Sensor.commmunity ("Luftdaten") and Adafruit IO are enabled
                if comms_failure == False:
                    with open('/home/enviropi/enviro-monitor/log', 'w') as f: ##<> Enter location of your log file
                        f.write('Enviro Script Alive')
                if enable_luftdaten: # Send data to Sensor.commmunity ("Luftdaten") if enabled
                    ...
                    luft_noise_values = [] #Reset Sensor.community ("Luftdaten") Noise Values List after each attempted transmission
                    #logging.info("Luftdaten Response: {}\n".format("ok" if luft_resp else "failed"))
                    data_sent_to_luftdaten_or_aio = True
                    if luft_resp:
                        print("Sensor.community (''Luftdaten'') update successful. Waiting for next capture cycle")
                    else:
                        print("Sensor.community (''Luftdaten'') update unsuccessful. Waiting for next capture cycle")
                else:
            ...
            # 20 minutes ago
            if first_climate_reading_done and (time.time() - barometer_log_time) >= 5:
            ...
            if run_time > startup_stabilisation_time: # Wait until the gas sensors have stabilised before providing
                ...
                if time_since_long_update >= long_update_delay:
                    ...
                    if (enable_indoor_outdoor_functionality and indoor_outdoor_function == 'Indoor'
                        ...
                        # outdoor data via Sensor.community ("Luftdaten") or Adafruit IO
                        long_update_toggle = not long_update_toggle
                        if long_update_toggle: # Only capture outdoor data via Sensor.community ("Luftdaten") or Adafruit IO on every second cycle
...
#                                                                      [[outdoor_data["P2.5"][1], 1]]
                    ...
                    with open('/home/enviropi/enviro-monitor/log-persistent', 'w') as f: ##<> Enter location of your config file
                        f.write(json.dumps(persistent_data_log))
                    if "Forecast" in mqtt_values:
                        mqtt_values.pop("Forecast") # Remove Forecast after sending it to mqtt so that
                    ...
                    if latest_mender_software_version != startup_mender_software_version or\
                        ...
                        if enable_send_data_to_homemanager or enable_receive_data_from_homemanager:
                            client.loop_stop()
                        time.sleep(10)
                        os.system('sudo systemctl restart enviro-plus.service')
                    print('Waiting for next capture cycle')

            # Sensor.community ("Luftdaten") and/or Adafruit IO Communications Check. Note that aio_resp and luft_resp are both TRUE on startup,
            ...
            if enable_adafruit_io and enable_luftdaten:
                if aio_resp or luft_resp: # Set time when a successful Sensor.community ("Luftdaten") or Adafruit IO response is received,
                    # if both Sensor.community ("Luftdaten") and Adafruit IO are enabled
                ...
                if aio_resp:
                    successful_comms_time = time.time()
            elif enable_luftdaten and not enable_adafruit_io: # Set time when a successful Sensor.community ("Luftdaten") response is received,
                # if only Sensor.community ("Luftdaten") is enabled
                if luft_resp:
                    successful_comms_time = time.time()
            else: # Set time if both Adafruit IO and Sensor.community ("Luftdaten") are disabled so that comms failure is never triggered
                successful_comms_time = time.time()
...
# MQTT integration instructions: (e.g. Homeassistant)
# name: "EnviroPlus Temperature"
#       state_topic: "<your 'mqtt_topic' as per config.json - note either indoor or outdoor >"
#       value_template: "{{ value_json.Temp }}"
#       unit_of_measurement: "°C"
#       state_class: "measurement"
#       unique_id: "sensor.enviroplus.temp"
#       device_class: "temperature"
#       device:
#         manufacturer: "Pimoroni"
#         identifiers: "EnviroPlus"
#         name: "EnviroPlus"
#         model: "Ennvironment Quality"
# name: "EnviroPlus Sound Level"
#       state_topic: "<your 'mqtt_topic' as per config.json - note either indoor or outdoor >"
#       value_template: "{{ value_json.Noise }}"
#       unit_of_measurement: "dB"
#       state_class: "measurement"
#       unique_id: "sensor.enviroplus.db"
#       device_class: "sound_pressure"
#       device:
#         manufacturer: "Pimoroni"
#         identifiers: "EnviroPlus"
# name: "EnviroPlus Humidity"
#       state_topic: "<your 'mqtt_topic' as per config.json - note either indoor or outdoor >"
#       value_template: "{{ value_json.Hum[0] }}"
#       unit_of_measurement: "%"
#       state_class: "measurement"
#       unique_id: "sensor.enviroplus.humidity"
#       device_class: "humidity"
#       device:
#         manufacturer: "Pimoroni"
#         identifiers: "EnviroPlus"
# name: "EnviroPlus Pressure"
#       state_topic: "<your 'mqtt_topic' as per config.json - note either indoor or outdoor >"
#       value_template: "{{ value_json.Bar[0] }}"
#       unit_of_measurement: "mbar"
#       state_class: "measurement"
#       unique_id: "sensor.enviroplus.pressure"
#       device_class: "pressure"
#       device:
#         manufacturer: "Pimoroni"
#         identifiers: "EnviroPlus"
# name: "EnviroPlus Light"
#       state_topic: "<your 'mqtt_topic' as per config.json - note either indoor or outdoor >"
#       value_template: "{{ value_json.Lux }}"
#       unit_of_measurement: "lx"
#       state_class: "measurement"
#       unique_id: "sensor.enviroplus.light"
...
#       state_class: "measurement"
#       unique_id: "sensor.enviroplus.nh3"
...
#       state_class: "measurement"
#       unique_id: "sensor.enviroplus.co"
...
#       state_class: "measurement"
#       unique_id: "sensor.enviroplus.no2"
#       device_class: "nitrogen_dioxide"
#       device:
#         manufacturer: "Pimoroni"
#         identifiers: "EnviroPlus"
# name: "EnviroPlus Forecast" (i Haven't got this to work)
#       state_topic: "<your 'mqtt_topic' as per config.json - note either indoor or outdoor>"
#       value_template: "{{ value_json.Forecast[3] }}"
#       unit_of_measurement: "µg/m³"
#       state_class: "measurement"
#       unique_id: "sensor.enviroplus.forecast"
#       device_class: "enum"
#       device:
#         manufacturer: "Pimoroni"
#         identifiers: "EnviroPlus"
# name: "EnviroPlus LCD"
#     unique_id: "enviroplus_lcd"
#     state_topic: "<your 'mqtt_topic' as per config.json - note either indoor or outdoor>/state/lcd"
#     command_topic: "<your 'mqtt_topic' as per config.json - note either indoor or outdoor>/cmd/lcd"
#     payload_on: "ON"
#     payload_off: "OFF"
#     state_on: "ON"
#     state_off: "OFF"
...
# Acknowledgements
# Based on code from:
# https://github.com/roscoe81/enviro-monitor ESPECIALLY HUGE THANKS!!!
